...
const LIKE_NOTICE_KINDS = new Set(['content-like', 'content-collect', 'comment-like', 'reply-like']);
const DELETED_LIKES_KEY_PREFIX = '@chat_likes_deleted_v1:';
...
export default function ChatLikesScreen() {
  ...
  const deletedKey = user?.account ? `${DELETED_LIKES_KEY_PREFIX}${user.account}` : '';
  ...
  const loadNotices = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const list = Array.isArray(result?.data)
        ? result.data
            .filter((item) => LIKE_NOTICE_KINDS.has(String(item.kind || '').trim()))
    ...
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      ...
      <ThemedView style={styles.root}>
        <View style={styles.header}>
          ...
          <ThemedText style={styles.headerTitle}>收到的赞和收藏</ThemedText>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              {loading ? <AppActivityIndicator label="正在加载" /> : <ThemedText style={styles.emptyText}>{error || '暂无收到的赞和收藏'}</ThemedText>}
            </View>
          }
          renderItem={({ item }) => (
            <Swipeable
              ...
              <Pressable style={styles.row} onPress={() => openNotice(item)}>
                ...
                <View style={styles.main}>
                  ...
                  <ThemedText numberOfLines={2} style={styles.content}>{item.message || '赞了或收藏了你发布的内容'}</ThemedText>
